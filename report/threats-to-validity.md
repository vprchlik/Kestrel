...
9. **Host variance.** `taskset`, recorded load average, N=30, median/IQR.
    ...
    **nonzero `/proc/stat` steal**, and USER_HZ=100 (10 ms steal ticks).
    D-0055's performance governor is `unavailable`. Report numbers for
    the paper come from a dedicated Ubuntu host (native, performance
    governor, SMT off, steal 0); this workspace is bring-up and
    correctness gates only. Fresh interleaved pair (git `9678270`): steal
    was 0 on 119/120 recorded trials, Spearman(steal, E0→E4) ≈ 0, and the
    one stolen trial was not in the slow quartile. Sub-tick host jitter
    remains (fast-boot E0→E4 still misses the two-batch bar;
    Spearman(run_order, E0→E4) = 0.55). Steal=0 is necessary, not
    sufficient (USER_HZ=100).
